...
from config import (
    ...
    PROMPT_VER, LLM_MODEL, TEMPERATURE, DOC_PATH,
    EMBED_MODEL
)
...
print("■ config.py의 최적 설정으로 검증")
print("-" * 78)
print(f"📋 현재 설정: chunk={CHUNK_SIZE}, k={TOP_K}, v={PROMPT_VER}")
print()

baseline = run("최적설정", CHUNK_SIZE, CHUNK_OVERLAP, TOP_K, MIN_SCORE, PROMPT_VER, TESTS)

print("\n" + "="*60)
if baseline["rate"] == 100:
    print("✅ config.py 설정 검증 완료! (정답율 100%)")
else:
    print(f"⚠️  정답율 {baseline['rate']:.0f}% (재검토 필요)")
print("="*80)